/**
 * Created by yangqiaosheng on 2018/5/21.
 */
var Windy = function (params) {
	...
	var FRAME_RATE = 15, FRAME_TIME = 1000 / FRAME_RATE;
	...
	var bilinearInterpolateVector = function bilinearInterpolateVector(x, y,
			g00, g10, g01, g11) {
		var rx = 1 - x;
		var ry = 1 - y;
	...
	var createBuilder = function createBuilder(data) {
		var uComp = null, vComp = null, temp = null;
		...
		if (row = grid[fj]) {
			...
			if (isValue(g00) && isValue(g10) && (row = grid[cj])) {
				...
				if (isValue(g01) && isValue(g11)) {
					return builder.interpolate(i - fi, j - fj, g00, g10, g01,
							g11);
				}
			}
		}
	...
	var createField = function createField(columns, bounds, callback) {
		...
		field.randomize = function(o) {
			...
			var safetyNet = 0;
			do {
				x = Math.round(Math.floor(Math.random() * bounds.width)
						+ bounds.x);
				y = Math.round(Math.floor(Math.random() * bounds.height)
						+ bounds.y);
	...
	if (params.projection === 'EPSG:4326') {
		invert = function invert(x, y, windy) {
			...
			var lat = rad2deg(windy.north) + y / windy.height
					* rad2deg(mapLatDelta);
			var lon = rad2deg(windy.west) + x / windy.width
					* rad2deg(mapLonDelta);
			return [ lon, lat ];
		};
	} else {
		invert = function invert(x, y, windy) {
			var mapLonDelta = windy.east - windy.west;
			var worldMapRadius = windy.width / rad2deg(mapLonDelta) * 360
					/ (2 * Math.PI);
			var mapOffsetY = worldMapRadius
					/ 2
					* Math.log((1 + Math.sin(windy.south))
							/ (1 - Math.sin(windy.south)));
			var equatorY = windy.height + mapOffsetY;
			var a = (equatorY - y) / worldMapRadius;
			var lat = 180 / Math.PI
					* (2 * Math.atan(Math.exp(a)) - Math.PI / 2);
			var lon = rad2deg(windy.west) + x / windy.width
					* rad2deg(mapLonDelta);
			return [ lon, lat ];
		};
	...
	var interpolateField = function interpolateField(grid, bounds, extent,
		...
		var mapArea = (extent.south - extent.north)
			...
			for (var y = bounds.y; y <= bounds.yMax; y += 2) {
				...
				if (coord) {
					...
					if (isFinite(λ)) {
						...
						if (wind) {
							wind = distort(projection, λ, φ, x, y,
									velocityScale, wind, extent);
	...
	var animate = function animate(bounds, field, extent) {
		...
		function windTemperatureColorScale(minTemp, maxTemp) {
			// var result = ["rgb(36,104, 180)", "rgb(60,157, 194)",
			// "rgb(128,205,193 )", "rgb(151,218,168 )", "rgb(198,231,181)",
			// "rgb(238,247,217)", "rgb(255,238,159)", "rgb(252,217,125)",
			// "rgb(255,182,100)", "rgb(252,150,75)", "rgb(250,112,52)",
			// "rgb(245,64,32)", "rgb(237,45,28)", "rgb(220,24,32)",
			// "rgb(180,0,35)"];
			// var result = [ "rgb(0, 0, 255)" ];
			var result = [ "rgb(62, 147, 197)" ];
			result.indexFor = function(m) {
				return Math.max(0, Math.min(result.length - 1, Math
						.round((m - minTemp) / (maxTemp - minTemp)
								* (result.length - 1))));
			};
			return result;
		}

		var colorStyles = windTemperatureColorScale(MIN_TEMPERATURE_K,
				MAX_TEMPERATURE_K);
		...
		var mapArea = (extent.south - extent.north)
				* (extent.west - extent.east);
		var particleCount = Math.round(bounds.width * bounds.height
				* PARTICLE_MULTIPLIER * Math.pow(mapArea, 0.24));
		if (isMobile()) {
		...
		function evolve() {
			...
			particles.forEach(function(particle) {
				if (particle.age > MAX_PARTICLE_AGE) {
					field.randomize(particle).age = ~~(Math.random()
							* MAX_PARTICLE_AGE / 2);
				}
				var x = particle.x;
				var y = particle.y;
	...
	var shift = function shift(dx, dy) {
		var canvas = params.canvas, w = canvas.width, h = canvas.height, ctx = canvas
				.getContext("2d");
		if (w > dx && h > dy) {
			var clamp = function clamp(high, value) {
				return Math.max(0, Math.min(high, value));
			};
			var imageData = ctx.getImageData(clamp(w, -dx), clamp(h, -dy),
					clamp(w, w - dx), clamp(h, h - dy));
