...
var TailedSquadCombat = function (_AttackArrow) {
  ...
  TailedSquadCombat.prototype.generate = function generate() {
    try {
      ...
      if (count < 2) {
        ...
        var pnts = this.getPoints();
        var tailPnts = this.getTailPoints(pnts);
        var firstTailPnts = tailPnts[0];
        var endTailPnts = tailPnts[tailPnts.length - 1];
        var headPnts = this.getArrowHeadPoints(pnts, firstTailPnts, endTailPnts);
        var neckLeft = headPnts[0];
        var neckRight = headPnts[4];
        var bodyPnts = this.getArrowBodyPoints(pnts, neckLeft, neckRight, this.tailWidthFactor);
        var _count = bodyPnts.length;
        var leftPnts = [firstTailPnts].concat(bodyPnts.slice(0, _count / 2));
        leftPnts.push(neckLeft);
        var rightPnts = [endTailPnts].concat(bodyPnts.slice(_count / 2, _count));
        rightPnts.push(neckRight);
        leftPnts = getQBSplinePoints(leftPnts);
        rightPnts = getQBSplinePoints(rightPnts);
        this.setCoordinates([leftPnts.concat(headPnts, rightPnts.reverse(), tailPnts.reverse(), [ leftPnts[0]])]);
      }
    } catch (e) {
      console.log(e);
    }
  ...
  TailedSquadCombat.prototype.getTailPoints = function getTailPoints(points) {
    var allLen = getBaseLength(points);
    var tailWidth = allLen * this.tailWidthFactor;
    var tailLeft = getThirdPoint(points[1], points[0], 3*HALF_PI/4, tailWidth, false);
    var tailRight = getThirdPoint(points[1], points[0], 3*HALF_PI/4, tailWidth, true);
    var len = tailWidth * this.swallowTailFactor;
    var swallowTailPnt = getThirdPoint(points[1], points[0], 0, len, true);
    return getCurvePoints(0.3, [tailLeft, swallowTailPnt, tailRight]);
  };
