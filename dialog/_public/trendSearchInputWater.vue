      ...
      <div class="dropdown-wrapper" v-show="showDropDown">
        <div class="widge-title-tab">
          <span class="tab-item"
                ...
                @click="handleChooseKqzd" v-if="'kqzd' == stationType">空气站点</span>
          <span class="tab-item"
                :class="{cur:'shjzd'==curType}"
                @click="handleChooseShjzd" v-if="'shjzd' == stationType">水环境站点</span>
          <span class="tab-item"
        ...
        <!-- 水环境站点 -->
        <div class="dropdown-content detailPanel"
             :class="{'dis_none': 'shjzd' != curType}">
          <ul v-if="shjzdList.length === 0" class="air-site">
            <li>{{loadingMsg}}</li>
          </ul>
          <ul v-else class="air-site">
            <li v-for="item in shjzdList" @click="handleSelectedShjzd(item)" :title="descName(item)">{{descName(item)}}
            </li>
          </ul>
        </div>
        <!-- 行政区域 -->
...
<script type="text/jsx">
  ...
  export default {
    ...
    methods: {
      getType(){
        this.curType = this.stationType = (this.params.type === 'hldm' ? 'shjzd' : 'kqzd');
      },
      descName(item){
      ...
      handleQueryChange(value){
        ...
        this.timer = window.setTimeout(() => {
          ...
          mapLayerHttp.getContrastSearch(params).then(response => {
            ...
            } else if(params.type === 'shjzd') {
              this.shjzdList = newDataList;
            } else {
