<template>
	<div class="sub-tab-content">
	  ...
	  <h2 class="table-caption">主要污染物产生及排放情况</h2>
        <section class="table-list mb_15" v-for="(item,index) in distinctMapList">
        	<template v-if="item.childCode != 'archives50302child01'">
	        ...
	        <table class="table small-padding table-complex table-bordered light-blue-th">
	          <thead>
	          	<tr v-if="index != 0">
	          		...
	          		<th>计量单位</th>
	          		<th>代码</th>
	          		<th colspan="2" v-for="(name,index) in item.nameArr">{{ name }}</th>
	          	</tr>
	          	...
	          	<tr v-if="index != 0">
	          		...
	          		<th>乙</th>
	          		<th>丙</th>
	          		<th colspan="2" v-for="(name,index) in item.nameArr">{{(index+1)}}</th>
	          	</tr>
	          </thead>
	          <tbody>
	          	<tr v-if="loading&&item.dataArr&&item.dataArr.length==0">
	          		<td class="al_center" colspan="2">{{constants.no_data_text}}</td>
	          	</tr>
	          	<tr  v-for="data in item.dataArr">
	          		...
	          		<td class="al_center" colspan="2" v-for="name in item.nameArr">{{data[name]}}</td>
	          	</tr>
	          </tbody>
	        </table>
	        </template>
