	...
	<div class="sub-tab-content">
	  <div class="table-list-wrapper">
        ...
        <tbody>
          ...
          <tr v-else v-for="(item , index) in dataList">
...
export default{
  ...
  mounted(){
    this.getSourceInfoFactorDataEs();
  },
	methods:{
    getSourceInfoFactorDataEs(){
      console.log('this.esSopObj:', this.esSopObj)
      let params = {
        sopId: this.esSopObj.id
      }
      polluteAsourceFileHttp.getSourceInfoFactorDataEs(params)
        .then(result => {
          if(result.data && result.data.length > 0){
            ...
            cosole.log(this.dataList, this.nameList)
          }
          this.loading = true;
        }).catch((error) => {
        this.loading = true;
