  ...
  <div class="popbox-detail-content">
    <table class="table table-bordered light-gray-th table-baseInfo">
      ...
      <tr>
        ...
        <th style="width:8%">建厂时间</th>
        <td style="width: 22%;">{{itemData.customTreat.buildTime}}</td>
      </tr>
      <tr>
        <th>污水处理设施类型</th>
        <td>
          <zt-dictionary :itemCode="constants.sop_handlefac_type" :code="itemData.customTreat.handlefacType"></zt-dictionary>
        </td>
        <th>运营单位名称</th>
        <td colspan="2">{{itemData.customTreat.operatorName}}</td>
        <th>污水处理级别</th>
        <td>
          <zt-dictionary :itemCode="constants.sop_handle_lever" :code="itemData.customTreat.handleLever"></zt-dictionary>
        </td>
      </tr>
      <tr>
        <th>污水处理方法</th>
        <td>
          <zt-dictionary :itemCode="constants.sop_handle_way_water" :code="itemData.customTreat.handleMethod"></zt-dictionary>
        </td>
        <th>排水去向类型</th>
        <td colspan="2">
          <zt-dictionary :itemCode="constants.sop_direction_type" :code="itemData.customTreat.directionType"></zt-dictionary>
        </td>
        <th>受纳水体</th>
        <td>
          <zt-dictionary :itemCode="constants.sop_water_area" :code="itemData.customTreat.receivingWater"></zt-dictionary>
        </td>
      </tr>
      <tr>
        <th>污泥厌氧消化装置</th>
        <td>
          {{itemData.customTreat.digestionDevice | yeaOrNoConvert}}
        </td>
        <th>再生水处理工艺</th>
        <td colspan="2">
          {{itemData.customTreat.hasReuseWater | yeaOrNoConvert}}
        </td>
        <th>锅炉</th>
        <td>
          {{itemData.customTreat.hasBoiler | yeaOrNoConvert}}
        </td>
      </tr>
      <tr>
        <th>执行排放标准</th>
        <td>
          {{itemData.customTreat.standard}}
        </td>
        <th>标准限值级别</th>
        <td colspan="4">
          {{itemData.customTreat.standardLimit}}
        </td>
      </tr>
      </tbody>
    </table>
