<template>
  ...
  <div class="tab-content" v-if="!noData">
    ...
    <div class="panel-table-heaer">
      <time> {{detailData.time ? moment(new Date(detailData.time.replace(/-/g, '/'))).format('MM月DD日HH点') : ''}}<em class="label pollute-fine ml_5">{{detailData.processType}}</em></time>
  ...
  export default {
    ...
    mounted () {
      ...
      this.chart.on('click', chartParams => {
        ...
        let params = {
          pollutionId: this.item.pollutionId,
    ...
    methods: {
      ...
      formatData: function (params) {
        ...
        for (var i = 0; i < dataList.length; i++) {
          ...
          if (itemTime.length === 2 && Number(itemTime[1]) > 10) {
            xAxisArray.push(itemTime[0] + '第' + itemTime[1] + '周')
          } else {
